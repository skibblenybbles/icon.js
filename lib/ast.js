...
var //---------------
    ...
    $nan = partial($, 'NaN'),
    $infinity = partial($, 'Infinity'),

    //----------------------
    //    Literal Values
    //----------------------

    // Is the given value a literal boolean, string, finite number or a
    // constant named by a global symbol?
    ...
    // repeatedly evaluate a special form or expression.
    ...
    // Forin special forms evaluate an iterable expression and assign the value
    // of its keys to a symbol for each iteration in which a special form or
    // expression is evaluated.
    $forin = function(name, iterable, body) {
        return [
            'forin',
            isSym(name) ?
                $(name) :
    ...
    $lambda = function(params) {
        return [
            ...
            invoke(
                $block,
                map(slice(arguments, 1), function(line) {
                    return (
                        isVal(line) ?
                            $val(line) :
                            line);
    ...
    // A member expression retrieves the value of a named member of an
    // evaluated expression.
    $m = function(object, name) {
        return [
            'member',
            isSym(object) ?
                $(object) :
    ...
    // An index expression retrieves the value of an evaluated key expression
    // member from an evaluated expression.
    $i = function(object, key) {
        return [
            'index',
            isSym(object) ?
                $(object) :
